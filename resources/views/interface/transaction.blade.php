                            ...
                            @foreach ($products as $product)
                                <tr>
                                    ...
                                    @if (auth()->user()->is_admin == 1)
                                        ...
                                        <td>
                                            <a href="/viewDetailTransaction?id_tr={{ $transaction->id }}&id_product={{ $product->id }}"
                                                class="btn btn-secondary me-2">Detail</a>
                                        </td>
                                    @else
                                        @if ($transaction->is_paid != 'Success' and $transaction->is_paid != 'Process')
                                            <td>
                                                <a href="/viewEditTransaction?id_tr={{ $transaction->id }}&id_product={{ $product->id }}"
                                                    class="btn btn-secondary me-2">Edit</a>
                                            </td>
                                            <td>
                                                <a href="{{ route('deleteTransaction', ['id' => $transaction->id]) }}"
